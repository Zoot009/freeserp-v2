 ...
 *
 * A sibling of /audit-suite's and /tracking-suite's, kept separate so the three
 * campaigns report distinct funnels — `page: "ai_rank_tracker"` is what
 * separates them downstream. page.tsx itself stays a server component so the
 * whole page is static HTML; this and the popup are the only islands that need
 * the browser.
 *
 ...
 *
 * Skins live in ads.css (.airt-btn-*) rather than here, so the page can be
    ...
    <a
      ...
      onClick={(e) =>
        trackLandingAndFlush("signup_cta_click", {
          placement,
          page: "ai_rank_tracker",
          href: e.currentTarget.getAttribute("href"),
        })
      }
      className={`airt-btn airt-btn-${size} airt-btn-${variant} ${className}`}
    >
      {label}
      {arrow === "up-right" ? (

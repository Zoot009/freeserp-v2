      ...
      <div className="fs-maps-shot" style={{ maxWidth: 1320, margin: "0 auto", padding: "0 60px" }}>
        <Reveal>
          ...
          >
            <Image
              src={SCAN_SHOT.src}
              alt="local rank tracker — FreeSERP local visibility for a business in New York, with ranked search points on the map and the competitors holding the top-3 spots"
              title="FreeSERP local visibility"
              width={SCAN_SHOT.w}
              height={SCAN_SHOT.h}
              priority
...
export function StepCards() {
  return (
    ...
    >
      <SectionHead
        tag="Step By Step"
        title="How local rank tracking works"
        sub="Pick the place your customers search from, pick the device they search on, and let the checks run. Every position is a real query in that market, not a national number relabelled."
      />
      <div
